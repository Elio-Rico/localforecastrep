...
This do file uses the file in the folder data_output. It merges the different
data bases from gdp and inflation. It generates several variables for the 
analysis, such as Forecast Revision and Forecast Error. It cleans the 
data base (we discard observations with fewer observations per id, etc.)
...
* pc
global data "/Users/eliob/Dropbox/4Foreign vs local expectations/Data/data_output"
global path "/Users/eliob/Dropbox/4Foreign vs local expectations/Data/stata"
global empRes "/Users/eliob/Dropbox/4Foreign vs local expectations/EmpiricalResults"
global navid "/Users/eliob/Dropbox/4Foreign vs local expectations/Navid's File/"
global gravity "/Users/eliob/Dropbox/4Foreign vs local expectations/Data/Gravity Data/"

* mac
global data "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_output"
global path "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/stata"
global empRes "/Users/ebollige/Dropbox/4Foreign vs local expectations/EmpiricalResults"
global navid "/Users/ebollige/Dropbox/4Foreign vs local expectations/Navid's File/"
global gravity "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/Gravity Data/"


cd "$path"

use "$data/df_gdp_stata.dta", clear
...
g gdp_imf_current = gdprpc_imf if current == 1
g gdp_imf_future = gdprpc_imf if current == 0

g year_forecast_current_gdp = year_forecast if current == 1
g year_forecast_future_gdp = year_forecast if current == 0
 
keep gdp_current gdp_future gdp_imf_current gdp_imf_future institution country date current local local_2 year_forecast_current_gdp year_forecast_future_gdp

collapse gdp_current gdp_future gdp_imf_current gdp_imf_future local local_2 year_forecast_current_gdp year_forecast_future_gdp, by(country date institution)
...
save "$data/df_gdp_stata_cleaned_V2.dta", replace
...
use "$data/df_cpi_stata.dta", clear
...
g cpi_imf_current = cpi_imf if current == 1
g cpi_imf_future = cpi_imf if current == 0

g cpi_imf_harm_current = cpi_imf_harm if current == 1
g cpi_imf_harm_future = cpi_imf_harm if current == 0
...
keep cpi_current cpi_future cpi_imf_current cpi_imf_future cpi_imf_harm_current cpi_imf_harm_future institution country date current year_forecast_current_cpi year_forecast_future_cpi

sort country date current

collapse cpi_current cpi_future cpi_imf_current cpi_imf_future cpi_imf_harm_current cpi_imf_harm_future year_forecast_current_cpi year_forecast_future_cpi, by(country date  institution)


save "$data/df_cpi_stata_cleaned_V2.dta", replace
...
use "$data/df_unemp_stata.dta", clear
...
g unemp_current = unemp if current == 1
g unemp_future = unemp if current == 0


g unemp_imf_current = unemp_imf if current == 1
g unemp_imf_future = unemp_imf if current == 0




g year_forecast_current_unemp = year_forecast if current == 1
g year_forecast_future_unemp = year_forecast if current == 0


keep unemp_current unemp_future unemp_imf_current unemp_imf_future institution country date current year_forecast_current_unemp year_forecast_future_unemp

collapse unemp_current unemp_future unemp_imf_current unemp_imf_future year_forecast_current_unemp year_forecast_future_unemp , by(country date  institution)


save "$data/df_unemp_stata_cleaned_V2.dta", replace
...
use "$data/df_sir_stata.dta", clear
...
g sir_current = sir if current == 1
g sir_future = sir if current == 0


g sir_imf_current = sir_imf if current == 1
g sir_imf_future = sir_imf if current == 0




g year_forecast_current_sir = year_forecast if current == 1
g year_forecast_future_sir = year_forecast if current == 0


g year_forecast_current_sir_num = 3 if year_forecast_current_sir == "In 3m"
g year_forecast_future_sir_num = 12 if year_forecast_future_sir == "In 12m"

keep sir_current sir_future sir_imf_current sir_imf_future institution country date current year_forecast_current_sir year_forecast_future_sir ///
year_forecast_current_sir_num year_forecast_future_sir_num

collapse sir_current sir_future sir_imf_current sir_imf_future year_forecast_current_sir_num year_forecast_future_sir_num , by(country date  institution)

g year_forecast_current_sir = "In 3m"
g year_forecast_future_sir = "In 12m"

drop year_forecast_current_sir_num
drop year_forecast_future_sir_num

save "$data/df_sir_stata_cleaned_V2.dta", replace
...
use "$data/df_lir_stata.dta", clear
...
g lir_current = lir if current == 1
g lir_future = lir if current == 0


g lir_imf_current = lir_imf if current == 1
g lir_imf_future = lir_imf if current == 0




g year_forecast_current_lir = year_forecast if current == 1
g year_forecast_future_lir = year_forecast if current == 0


g year_forecast_current_lir_num = 3 if year_forecast_current_lir == "In 3m"
g year_forecast_future_lir_num = 12 if year_forecast_future_lir == "In 12m"

keep lir_current lir_future lir_imf_current lir_imf_future institution country date current year_forecast_current_lir year_forecast_future_lir ///
year_forecast_current_lir_num year_forecast_future_lir_num

collapse lir_current lir_future lir_imf_current lir_imf_future year_forecast_current_lir_num year_forecast_future_lir_num , by(country date  institution)

g year_forecast_current_lir = "In 3m"
g year_forecast_future_lir = "In 12m"

drop year_forecast_current_lir_num
drop year_forecast_future_lir_num

save "$data/df_lir_stata_cleaned_V2.dta", replace
...
use "$data/df_inv_stata.dta", clear
...
g inv_current = inv if current == 1
g inv_future = inv if current == 0


g inv_imf_current = inv_imf if current == 1
g inv_imf_future = inv_imf if current == 0


g year_forecast_current_inv = year_forecast if current == 1
g year_forecast_future_inv = year_forecast if current == 0


keep inv_current inv_future inv_imf_current inv_imf_future institution country date current year_forecast_current_inv year_forecast_future_inv

collapse inv_current inv_future inv_imf_current inv_imf_future year_forecast_current_inv year_forecast_future_inv , by(country date  institution)


save "$data/df_inv_stata_cleaned_V2.dta", replace
...
use "$data/df_ca_stata.dta", clear
...
g ca_current = ca if current == 1
g ca_future = ca if current == 0


g year_forecast_current_ca = year_forecast if current == 1
g year_forecast_future_ca = year_forecast if current == 0


keep ca_current ca_future institution country date current year_forecast_current_ca year_forecast_future_ca

collapse ca_current ca_future  year_forecast_current_ca year_forecast_future_ca , by(country date  institution)


save "$data/df_ca_stata_cleaned_V2.dta", replace


* we will merge current account later on!
...
use "$data/df_gdp_stata_cleaned_V2.dta", clear 

merge 1:1 date country institution using "$data/df_cpi_stata_cleaned_V2.dta"
drop if _merge == 1

drop _merge


merge 1:1 date country institution using "$data/df_unemp_stata_cleaned_V2.dta"
drop _merge


merge 1:1 date country institution using "$data/df_sir_stata_cleaned_V2.dta"
drop _merge

merge 1:1 date country institution using "$data/df_lir_stata_cleaned_V2.dta"
drop _merge


merge 1:1 date country institution using "$data/df_inv_stata_cleaned_V2.dta"
drop _merge
...
merge 1:1 date country institution using "$data/df_ca_stata_cleaned_V2.dta"
drop if _merge == 2
drop _merge

drop country
rename country_or country
save "$data/data_withoutVintage.dta", replace

********************************************************************************
* ADD VINTAGE DATA


use "../vintage/WEO - final/Stata data/1. Final_dataset.dta" ,clear
...
replace var = "unemp" if descriptor == "Unemployment rate"
replace var = "cab" if descriptor == "Current account balance"


drop descriptor

* at the moment, we don't have data about cab yet // to be changed as soon as you changed R code
drop if var == "cab"
...
* we use vintage data from the fred for united states
...
preserve
	...
	save weo_apr_1999_`vintage'.dta, replace
	...
	* april
	preserve
		...
		if !_rc {
			...
			*reshape wide WEO_ , i(country) j(year)
			...
			save "weo_apr_`year'_`vintage'.dta", replace
		}
	...
	preserve
		...
		if !_rc {
			...
			save "weo_sep_`year'_`vintage'.dta", replace
...
foreach vintage in tp1 tp2 tp3 {
	...
	* APRIL
	use weo_apr_1999_`vintage'.dta, clear

	forval year = 2000(1)`yrmax' {

	
	capture confirm file weo_apr_`year'_`vintage'.dta
	if !_rc {
	
	append using weo_apr_`year'_`vintage'.dta
	...
	save weo_apr_vintage_`vintage'.dta, replace
	...
	* SEPTEMBER
	...
	capture confirm file weo_sep_`year'_`vintage'.dta
	if !_rc {
		append using weo_sep_`year'_`vintage'.dta
	}
	...
	save weo_sep_vintage_`vintage'.dta, replace

	forval year = 1999(1)`yrmax' {

	capture erase weo_apr_`year'_`vintage'.dta
	}
	forval year = 2000(1)`yrmax' {
	capture erase weo_sep_`year'_`vintage'.dta
	}
...
foreach vintage in tp1 tp2 tp3 {
use weo_sep_vintage_`vintage'.dta, clear
...
foreach hor in current future {
	...
	* reshape it from wide to long
	...
	save weo_sep_vintage_cpi_`hor'_`vintage'.dta,replace
...
foreach hor in current future {
	...
	* reshape it from wide to long
	...
	save weo_sep_vintage_gdp_`hor'_`vintage'.dta,replace

restore
}


* unemp current and future
...
foreach hor in current future {
	...
	* reshape it from wide to long
	...
	rename vintage_sep0 vintage_unemp_current_sep`vintage'

	rename vintage_sep1 vintage_unemp_future_sep`vintage'

	drop var
	
	keep year country_upper vintage_unemp_`hor'_sep`vintage'
	
	rename year year_forecast_`hor'_unemp

	save weo_sep_vintage_unemp_`hor'_`vintage'.dta,replace
...
use weo_apr_vintage_`vintage'.dta, clear
...
foreach hor in current future {
	...
	* reshape it from wide to long
	...
	save weo_apr_vintage_cpi_`hor'_`vintage'.dta,replace
...
foreach hor in current future {
	...
	* reshape it from wide to long
	...
	save weo_apr_vintage_gdp_`hor'_`vintage'.dta,replace
...
foreach hor in current future {
	...
	* reshape it from wide to long
	...
	rename vintage_apr0 vintage_unemp_current_apr`vintage'

	rename vintage_apr1 vintage_unemp_future_apr`vintage'

	drop var
	
	keep year country_upper vintage_unemp_`hor'_apr`vintage'
	
	rename year year_forecast_`hor'_unemp

	save weo_apr_vintage_unemp_`hor'_`vintage'.dta,replace
...
hence, we will use data from the file 2011 april file, for vintage series (earlier documents will be too close to own imf forecasts, but we can
experiment with this, of course. however, taking the files 1 year later also comes closer to what we did with the US vintage series.)
...
* APRIL VINTAGES
use "$data/data_withoutVintage.dta", replace
...
* gdp:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_gdp using weo_apr_vintage_gdp_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
* cpi:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_cpi using weo_apr_vintage_cpi_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
* unemp:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_unemp using weo_apr_vintage_unemp_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
* gdp:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_gdp using weo_sep_vintage_gdp_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
* cpi:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_cpi using weo_sep_vintage_cpi_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
* unemp:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_unemp using weo_sep_vintage_unemp_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
* we keep the us_vintage series separately in case we nevertheless use it as public signal or similar
/*
foreach month in sep apr{
	foreach vintage in tp1 tp2 tp3{
		replace vintage_gdp_current_`month'`vintage' = gdp_fred_vintage_current if country == "United States"
		replace vintage_gdp_future_`month'`vintage' = gdp_fred_vintage_future if country == "United States"
	}
}
*/
...
********************************************************************************
* standard stuff
foreach var in gdp_current gdp_future cpi_current cpi_future unemp_current unemp_future sir_current sir_future lir_current lir_future {
...
foreach var in gdp_future_share gdp_current_share cpi_future_share cpi_current_share unemp_future_share unemp_current_share  {
...
foreach var in gdp_future_nobs_pS gdp_current_nobs_pS cpi_future_nobs_pS cpi_current_nobs_pS unemp_future_nobs_pS unemp_current_nobs_pS {
...
order country country_num institution id date datem gdp_current gdp_future gdp_imf* cpi_current cpi_future cpi_imf* 
...
save "$data/DATA2_NewVintage.dta", replace


foreach var in gdp cpi unemp {
	foreach hor in current future {
		foreach month in sep apr {
			foreach vintage in tp1 tp2 tp3{
			
				capture erase "weo_`month'_vintage_`var'_`hor'_`vintage'.dta"
				
				capture erase "weo_`month'_vintage_`vintage'.dta"
...
rename institution institution_lower
g institution = upper(institution_lower)

merge 1:1 country institution date using ///
	"$navid/ExportsImports/imp_forecasts.dta"
drop if _merge == 2
drop _merge
merge 1:1 country institution date using ///
	"$navid/ExportsImports/exp_forecasts.dta"
drop if _merge == 2
drop _merge

drop institution
ren institution_lower institution
...
g country_upper = upper(country)
merge 1:1 country_upper institution date using ///
	"$data/../stata/current_account/ca_forecasts.dta"
drop institution
ren institution_lower institution
drop if _merge == 2
drop _merge
...
use "$data/DATA2_NewVintage.dta",clear


* ADD CRISIS DATA:
merge m:1 country year using "$navid/Crises/Crises - Harvard - modified.dta"
drop _merge
drop if institution == ""
...
save "$data/DATA2_crisis_NewVintage.dta", replace
...
clear all
import excel "$navid/Company Trees/Miscellaneous/Trees.xlsx", sheet("Sheet1") firstrow
...
save "$navid/Company Trees/Miscellaneous/Trees.dta", replace
...
clear all
use "$navid/Company Trees/Miscellaneous/Trees.dta", clear
...
clear all
use "$navid/Company Trees/Miscellaneous/subsidiary.dta"
merge 1:1 institution using "$navid/Company Trees/Miscellaneous/affiliate.dta"
drop _merge
merge 1:1 institution using "$navid/Company Trees/Miscellaneous/jointventure.dta"
drop _merge

save countries_trees.dta, replace

// now, we should merge this with the big company tree data set in order to have parents and other variables

clear all
use "$navid/Company Trees/Miscellaneous/Trees.dta"
...
merge 1:1 institution using "$navid/Company Trees/Miscellaneous/countries_trees.dta"
drop _merge
...
save trees_final.dta, replace

********************* Matching with the main dataset ***************************


clear all
use "$data/DATA2_crisis_NewVintage.dta"
...
merge m:1 institution using "trees_final.dta"
drop _merge
...
save "$data/data_final_newVintage.dta", replace
...
use "$data/../stata/inv/WEO_final_inv.dta",clear
...
preserve
	...
	g future = 0 if year == 2010
	replace future = 1 if year == 2011
	
	rename WEO`ticker'_apr vintage_apr
	

	save weo_apr_2011_`vintage'.dta, replace
...
foreach vintage in tp1 tp2 tp3 {
	...
	* APRIL
	use weo_apr_2011_`vintage'.dta, clear
...
foreach hor in current future {
	...
	* reshape it from wide to long
	...
	rename vintage_sep0 vintage_inv_current_sep`vintage'

	rename vintage_sep1 vintage_inv_future_sep`vintage'

	drop var
	
	keep year country_upper vintage_inv_`hor'_sep`vintage'
	
	rename year year_forecast_`hor'_inv

	save weo_sep_vintage_inv_`hor'_`vintage'.dta,replace
...
foreach hor in current future {
	...
	* reshape it from wide to long
	...
	rename vintage_apr0 vintage_inv_current_apr`vintage'

	rename vintage_apr1 vintage_inv_future_apr`vintage'

	drop var

	keep year country_upper vintage_inv_`hor'_apr`vintage'
	
	rename year year_forecast_`hor'_inv

	save weo_apr_vintage_inv_`hor'_`vintage'.dta,replace
...
* APRIL VINTAGES
use "$data/data_final_newVintage.dta", replace
...
* inv:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_inv using weo_apr_vintage_inv_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
* inv:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_inv using weo_sep_vintage_inv_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
save "$navid/Company Trees/data_final_newVintage.dta", replace
...
// current account vintages & forecasts
use "$path/current_account/WEO_final_ca.dta", clear
cd "$path/current_account"
* new descriptor variable
rename descriptor var
...
* no data until 2003, drop those variabels

drop WEO1999_apr - WEO2003_apr

* We use WEO2003_sep as the first dataset, we keep all data until 2002:
...
preserve
	...
	keep year country var WEO`ticker'_sep

	drop if year > 2003

	drop if WEO`ticker'_sep == .

	
	sort country var year  
	
	g future = 0 if year == 2002
	replace future = 1 if year == 2003
	
	rename WEO`ticker'_sep vintage_sep
	

	save weo_sep_2003_`vintage'.dta, replace
...
foreach vintage in tp1 tp2 tp3 {
	...
	* APRIL
	use weo_apr_2004_`vintage'.dta, clear

	forval year = 2005(1)`yrmax' {
	...
	* same for september updates
	* SEPTEMBER
	use weo_sep_2003_`vintage'.dta, clear

	rename vintage_sep vintage_sep
	...
	replace future = 0 if year < 2003

	save weo_sep_vintage_`vintage'.dta, replace

	forval year = 2003(1)`yrmax' {
...
foreach hor in current future {
	...
	keep if var == "Current account balance"
	...
	* reshape it from wide to long
	...
	rename vintage_sep0 vintage_ca_current_sep`vintage'

	rename vintage_sep1 vintage_ca_future_sep`vintage'

	drop var
	
	keep year country_upper vintage_ca_`hor'_sep`vintage'
	
	rename year year_forecast_`hor'_ca

	save weo_sep_vintage_ca_`hor'_`vintage'.dta,replace
...
foreach hor in current future {
	...
	* reshape it from wide to long
	...
	rename vintage_apr0 vintage_ca_current_apr`vintage'

	rename vintage_apr1 vintage_ca_future_apr`vintage'

	drop var

	keep year country_upper vintage_ca_`hor'_apr`vintage'
	
	rename year year_forecast_`hor'_ca

	save weo_apr_vintage_ca_`hor'_`vintage'.dta,replace
...
use "$navid/Company Trees/data_final_newVintage.dta", clear
...
* inv:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_ca using weo_apr_vintage_ca_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
* inv:
foreach hor in current future {
merge m:1 country_upper year_forecast_`hor'_ca using weo_sep_vintage_ca_`hor'_`vintage'.dta
drop if _merge == 2
drop _merge
...
gen FE_imp_current = imp_imf_current - imp_current
gen FE_imp_future = imp_imf_future - imp_future

gen FE_exp_current = exp_imf_current - exp_current
gen FE_exp_future = exp_imf_future - exp_future

egen idc = group(institution country)

sort idc date
gen FR_imp_current = imp_current[_n] - imp_current[_n-1]
gen FR_imp_future = imp_future[_n] - imp_future[_n-1]

gen FR_exp_current = exp_current[_n] - exp_current[_n-1]
gen FR_exp_future = exp_future[_n] - exp_future[_n-1]
...
import delimited "$path/../data_input/CLI-components-and-turning-points.csv" , clear
...
merge m:1 country datem using "$path/ri.dta"

drop if _merge == 2

drop _merge

*save "C:\Users\eliob\Dropbox\4Foreign vs local expectations\Data\data_input\datalongRecession.dta", replace 
*use "C:\Users\eliob\Dropbox\4Foreign vs local expectations\Navid's File\Company Trees\data_final.dta", clear
*merge m:1 country datem using "C:\Users\eliob\Dropbox\4Foreign vs local expectations\Data\data_input\datalongRecession.dta"
*drop if _merge == 2
*drop _merge
...
preserve
	...
	import delimited "$navid/RGDP and Industrial Production/Industrial Production data/IndustrialProduction_monthly.csv", clear
	...
	save indProd.dta, replace
	
restore


merge m:1 country datem using indProd.dta
drop if _merge == 2
drop _merge
...
foreach hor in current future {
foreach var in gdp cpi inv ca {
foreach vintage in tp1 tp2 tp3 {
	
	foreach month in apr sep {
	
		forval year = 1999(1)2021{
		
		capture erase "$path/weo_`month'_vintage_`var'_`hor'_`vintage'.dta"
		capture erase "$path/weo_`month'_vintage_`vintage'.dta"
		capture erase "$path/weo_`month'_vintage_`var'_`hor'.dta"
		
		
		capture erase "$path/current_account/weo_`month'_vintage_`var'_`hor'_`vintage'.dta"
		capture erase "$path/current_account/weo_`month'_vintage_`vintage'.dta"
		capture erase "$path/current_account/weo_`month'_vintage_`var'_`hor'.dta"
...
// EPU
import excel "$navid/Risk Measure/EPU/EPU_All_Country_Data.xlsx", sheet("EPU") firstrow clear
...
preserve
	keep datem GEPU*
	save "$navid/Risk Measure/EPU/GEPU.dta", replace
restore
...
merge m:1 datem using "$navid/Risk Measure/EPU/GEPU.dta"
...
save "$navid/Risk Measure/EPU/EPU.dta", replace
...
foreach c of local components{
	import excel "$navid/Risk Measure/ICRG/ICRG_T3B_for stata.xlsx", sheet("`c'") firstrow clear
	...
	save "$navid/Risk Measure/ICRG/`c'.dta", replace

}

// merge all in one

use "$navid/Risk Measure/ICRG/GovStab.dta", clear

local components Socioeco InvProf IntConf ExtConf Corrupt Military Religious Law Ethnic DemoAcct Bureau
foreach c of local components{
	merge 1:1 datem country using "$navid/Risk Measure/ICRG/`c'.dta"
	drop _merge
}

save "$navid/Risk Measure/ICRG/ICRGT3B.dta", replace

// erase individual datasets
local components GovStab Socioeco InvProf IntConf ExtConf Corrupt Military Religious Law Ethnic DemoAcct Bureau
foreach c of local components{
	erase "$navid/Risk Measure/ICRG/`c'.dta"
}
...
foreach c of local components{
	import excel "$navid/Risk Measure/ICRG/ICRG_T4B_for stata.xlsx", sheet("`c'") firstrow clear
...
use "$navid/Risk Measure/ICRG/ForDebt.dta", clear
...
save "$navid/Risk Measure/ICRG/ICRGT4B.dta", replace
...
use "$navid/Risk Measure/ICRG/ICRGT3B.dta", replace

merge 1:1 datem country using "$navid/Risk Measure/ICRG/ICRGT4B.dta"
drop _merge

save "$navid/Risk Measure/ICRG/ICRG_All.dta", replace
...
merge m:1 country datem using "$navid/Risk Measure/EPU/EPU.dta"
...
merge m:1 country datem using "$navid/Risk Measure/ICRG/ICRG_All.dta"
...
save "$navid/Company Trees/data_final_newVintage_withRisk.dta", replace
...
foreach hor in current future {
foreach var in gdp cpi unemp inv ca {
...
foreach hor in current future {
	foreach var in sir lir {
		
		g FE_`var'_`hor' = `var'_imf_`hor' - `var'_`hor'
...
cap drop vintage_sir_current vintage_sir_future vintage_lir_current vintage_lir_future
g vintage_sir_current = sir_imf_current
g vintage_sir_future = sir_imf_future
g vintage_lir_current = lir_imf_current
g vintage_lir_future = lir_imf_future
...
foreach x in s1 s2 s3 a1 a2 a3 {
	...
	foreach var in FE_gdp FE_cpi FE_unemp {
...
foreach var in sir lir  {
	foreach hor in current future {
		...
		gen abs_FE_`var'_`hor' = abs(FE_`var'_`hor' )
...
local varlist  sir lir 	/* gdp cpi sir lir */
local horlist current future 	/* current future */
foreach var in `varlist' {
    foreach hor in `horlist'{
	    gen labs_FE_`var'_`hor' = log(abs_FE_`var'_`hor')
		replace labs_FE_`var'_`hor' = log(0.01) if abs_FE_`var'_`hor' < 0.01
		replace labs_FE_`var'_`hor' = log(10) if abs_FE_`var'_`hor' > 10 & abs_FE_`var'_`hor' != .
	}
}
...
save "$navid/Company Trees/data_final_newVintage_2.dta", replace


use "$navid/Company Trees/data_final_newVintage_2.dta", replace

save "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/backup/data_final_newVintage_2_asof_20220309.dta", replace
...
use "$navid/Company Trees/data_final_newVintage_2.dta", clear

keep subsidiaryCountry* year country datem dateq institution Headquarters dist*

save "$navid/Company Trees/data_final_newVintage_2_toMergeGravity.dta" , replace
...
merge 1:1 country institution datem dateq Headquarters using "$gravity/gravity_temp.dta"
drop _merge
...
forval x = 1(1)121{
	...
	* replace the minimum distance subsidiary variable with the respective country if the distance matches 
	replace countryToMatch_`var' = subsidiaryCountry`x' if  `var'`x' == min_`var'_subs  & countryToMatch_`var' == ""
...
merge 1:1 country country_num institution id date datem dateq using "$gravity/gravity_temp2.dta"
drop _merge
...
save "$gravity/gravity_to_merge_`var'.dta", replace
...
merge m:1 year country closest_ctry_dist using "$gravity/gravity_to_merge_dist.dta"
drop if _merge == 2
drop _merge

merge m:1 year country closest_ctry_distw using "$gravity/gravity_to_merge_distw.dta"
drop if _merge == 2
drop _merge

merge m:1 year country closest_ctry_distcap using "$gravity/gravity_to_merge_distcap.dta"
drop if _merge == 2
drop _merge

merge m:1 year country closest_ctry_distwces using "$gravity/gravity_to_merge_distwces.dta"
drop if _merge == 2
drop _merge
...
import delimited "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/BIS_locationalStatistics_selected.csv", clear
	...
	preserve
		...
		*local var v2
		...
		if varname[1] == "totalclaims"{
			local savestring = "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/" + "`var'" + "claims.dta"
		}
		else {
		local savestring = "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/" + "`var'" + "liabs.dta"
...
use "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/v2claims.dta", clear

*append using "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/v3claims.dta"

forval x = 3(1)2400 {
	
	
	local fl = "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/v" + "`x'" + "claims.dta"
	disp("`fl'")
...
save "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/all_claims.dta"


use "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/v40liabs.dta", clear
forval x = 41(1)2400 {
	
	
	local fl = "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/v" + "`x'" + "liabs.dta"
	disp("`fl'")
	capture confirm file "`fl'"
	*display _rc
...
merge 1:1 reportingCountry counterpartyCountry dateq using "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/all_claims.dta"
...
preserve
	...
	save  "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_1.dta"
...
preserve
	...
	save  "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_2.dta"
...
erase "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/all_claims.dta"
...
merge m:1 country Headquarters dateq using "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_1.dta"
drop if _merge == 2
drop _merge



merge m:1 country Headquarters dateq using "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_2.dta"
drop if _merge == 2
drop _merge
...
use "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_1.dta", clear
...
save "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_1_`var'.dta", replace


use "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_2.dta", clear

rename Headquarters closest_ctry_`var'
rename totalliabilities2 totalliabilities2_`var'
rename totalclaims2 totalclaims2_`var'

save "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_2_`var'.dta", replace
...
merge m:1 country closest_ctry_`var' dateq using "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_1_`var'.dta"
drop if _merge == 2
drop _merge

merge m:1 country closest_ctry_`var' dateq using "/Users/ebollige/Dropbox/4Foreign vs local expectations/Data/data_input/bis_temp/BIS_all_2_`var'.dta"
drop if _merge == 2
drop _merge
